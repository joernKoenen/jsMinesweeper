    ...
    if (field[clickX] != undefined && field[clickX][clickY] != undefined) {
      ...
      if (view[clickX][clickY] == 0) {
        if (field[clickX][clickY] != 9) {
          view[clickX][clickY] = 1;
          revealleft--;
          //draw number
          switch (field[clickX][clickY]) {
            case 0:
              ctx.clearRect(clickX * scale + 1, clickY * scale + 1, scale - 2, scale - 2);
              revealZeros(clickX, clickY);
              break;
            case 1:
              ctx.fillStyle = "blue";
              ctx.clearRect(clickX * scale + 1, clickY * scale + 1, scale - 2, scale - 2);
              ctx.fillText(field[clickX][clickY], clickX * scale + scale / 2, clickY * scale + scale / 2);
              break;
            case 2:
              ctx.fillStyle = "green";
              ctx.clearRect(clickX * scale + 1, clickY * scale + 1, scale - 2, scale - 2);
          ...
          ctx.drawImage(bombImg, clickX * scale + 1, clickY * scale + 1, scale - 2, scale - 2);
  ...
  for (var ix = -1; ix <= 1; ix++) {
    for (var iy = -1; iy <= 1; iy++) {
      if ((ix != 0 || iy != 0) && field[x + ix] != undefined && field[x + ix][y + iy] != undefined && view[x + ix][y + iy] == 0) {
        ...
        revealleft--;
        //draw number
        switch (field[x + ix][y + iy]) {
          case 0:
            ctx.clearRect((x + ix) * scale + 1, (y + iy) * scale + 1, scale - 2, scale - 2);
            break;
          case 1:
            ctx.fillStyle = "blue";
            ctx.clearRect((x + ix) * scale + 1, (y + iy) * scale + 1, scale - 2, scale - 2);
            ctx.fillText(field[x + ix][y + iy], (x + ix) * scale + scale / 2, (y + iy) * scale + scale / 2);
            break;
          case 2:
            ctx.fillStyle = "green";
            ctx.clearRect((x + ix) * scale + 1, (y + iy) * scale + 1, scale - 2, scale - 2);
        ...
        if (field[x + ix][y + iy] == 0) revealZeros(x + ix, y + iy);
      }
    }
  }
}
